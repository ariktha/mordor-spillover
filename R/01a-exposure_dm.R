...
## Extracting coordinates from main_grappe and converting to sf object

main_gps <- main_grappe %>%
  dplyr::select(grappe, latitude, longitude) %>%
  distinct()

main_gps <- st_as_sf(main_gps, coords = c("longitude", "latitude"), crs = global_crs, remove = FALSE)
...
# Extracting coordinates from morb_gps_raw and converting to sf object

morb_gps <- morb_gps_raw %>%
  rename(grappe = gwu)
  
morb_gps <- st_as_sf(morb_gps, coords = c("longitude", "latitude"), crs = global_crs, remove = FALSE)

# Save data ---------------------------------------------------------------

saveRDS(main_grappe, here("data", "final", "main_grappe.rds"))
saveRDS(morb_grappe, here("data", "final", "morb_grappe.rds"))

# Save GPS data

saveRDS(main_gps, here("data", "final", "main_gps.rds"))
saveRDS(morb_gps, here("data", "final", "morb_gps.rds"))
