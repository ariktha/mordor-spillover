...
niger_union <- niger_shp %>% mutate(mordor_states = ifelse(ADM2_FR %in% c("Falmey", "Boboye", "Loga"), TRUE, FALSE)) %>% 
...
pop_raw <- readRDS(here("data", "final", "pop_crop.RDS"))
...
saveRDS(aggregated, here("data", "final", "hrsl_rings.RDS"))
