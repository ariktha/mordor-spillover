...
morb_pop_raw <- readRDS(here("data", "output", "hrsl_rings.RDS"))
morb_csi_raw <- readRDS(here("data", "output", "morb_csi.RDS"))
...
if(run_public){
  ...
  amr_dat_raw <- readRDS(file.path(path_data_clean, "amr_dat.rds"))
  ...
  amr_dat_raw <- readRDS(file.path(path_internal, "amr_dat.rds"))
...
ggplot(data = cor_tx_int_pwr2, aes(group = arm, color = arm)) +
  ...
  annotate("text", x = 0.5, y = 75, label = watermark_text_ml,
             hjust = 0.5, vjust = 0.5, col = watermark_color, cex = 16,
             fontface = "bold", alpha = 0.5)

ggsave(
  here("output", "figs-manuscript", "supp-figs", "supp_fig1.png"),
  bg = "white",
  width = 8,
  height = 5
)
...
saveRDS(loo_cor, here("data", "output", "loo-cor.RDS"))
...
ggplot() + geom_boxplot(data = loo_cor,
  ...
  annotate("text", x = 1.5, y = 0.25, label = watermark_text_ml,
             hjust = 0.5, vjust = 0.5, col = watermark_color, cex = 12,
             fontface = "bold", alpha = 0.5)

ggsave(
  here("output", "figs-manuscript", "supp-figs", "supp_fig2.png"),
  bg = "white",
  width = 7,
  height = 5
)
...
azithro_doses_plot <- ggplot(data = dat_fig3, 
  ...
  annotate("text", x = 10, y = 15, label = watermark_text_ml,
...
azithro_txint_plot <- ggplot(data = dat_fig3, aes(x = total_children / 1000, y = tx_int)) + 
  ...
  annotate("text", x = 10, y = 0.5, label = watermark_text_ml,
             hjust = 0.5, vjust = 0.5, col = watermark_color, cex = 8,
             fontface = "bold", alpha = 0.5)
...
ggsave(here("output", "figs-manuscript", "supp-figs", "supp_fig3.png"), 
...
p1 <- ggplot(morb_pop,
  ...
  annotate("text", x = 20, y = 15000, label = watermark_text_ml,
             hjust = 0.5, vjust = 0.5, col = watermark_color, cex = 10,
             fontface = "bold", alpha = 0.5)
...
p2 <- ggplot(morb_pop, aes(
  ...
  theme(legend.position = "bottom",
        panel.spacing.x = unit(0.6, "cm")) +
  ylim(c(0, 200))  + 
  annotate("text", x = 20, y = 100, label = watermark_text_ml,
             hjust = 0.5, vjust = 0.5, col = watermark_color, cex = 8,
             fontface = "bold", alpha = 0.5)
...
ggsave(here("output", "figs-manuscript", "supp-figs", "supp_fig4.png"), hrsl_plot, width = 8, height = 7, dpi = 600, bg = "white")
...
q1 <- ggplot(morb_csi, aes(y = tx_int, x = dist_csi, color = arm)) + 
  ...
  annotate("text", x = 3.8, y = 0.5, label = watermark_text,
             hjust = 0.5, vjust = 0.5, col = watermark_color, cex = 16,
             fontface = "bold", alpha = 0.5)
...
q2 <- ggplot(morb_csi, aes(x = dist_csi, y = avg_res, color = arm)) + 
  ...
  annotate("text", x = 4, y = 65, label = watermark_text_ml,
             hjust = 0.5, vjust = 0.5, col = watermark_color, cex = 12,
             fontface = "bold", alpha = 0.5)
...
ggsave(here("output", "figs-manuscript", "supp-figs", "supp_fig5.png"), 
...
other_abs_plot_with_ps <- cowplot::ggdraw(other_abs_plot_with_p) +
  cowplot::draw_label(
    watermark_text, 
    size = 80, 
...
ggsave(here("output", "figs-manuscript", "supp-figs", "supp_fig6-with-p.png"), other_abs_plot_with_ps, bg = "white", width = 11, height = 9, dpi = 300)
...
other_abs_plots <- cowplot::ggdraw(other_abs_plot_with_p) +
  cowplot::draw_label(
    watermark_text, 
    size = 80, 
    color = watermark_color,
...
ggsave(here("output", "figs-manuscript", "supp-figs", "supp_fig6.png"), other_abs_plots, bg = "white", width = 11, height = 9, dpi = 300)
...
dat_supp_fig7 <- morb_tx_int_raw %>%
  dplyr::filter(idw_power == 1, tx_type == "azithro") %>%
  dplyr::select(grappe, arm, tx_int) %>%
  left_join(morb_grappe %>% dplyr::select(grappe, n_children), by = "grappe") %>%
  left_join(
    mls_dat %>% dplyr::select(grappe, phase, avg_res),
    by = "grappe"
  )
...
ggsave(here("output", "figs-manuscript", "supp-figs", "supp_fig7.png"), 
