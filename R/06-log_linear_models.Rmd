...
if(run_public){
  morb_tx_raw <- readRDS(file.path(path_data_clean, "morb_tx_int.rds"))
  morb_rings_raw <- readRDS(file.path(path_data_clean, "morb_rings.rds"))
  
  amr_dat_raw <- readRDS(file.path(path_data_clean, "amr_dat.rds"))

} else {
  morb_tx_raw <- readRDS(file.path(path_internal, "morb_tx_int.rds"))
...
hrsl_rings_raw <- readRDS(here("data", "output", "hrsl_rings.RDS"))
csi_dist_raw <- readRDS(here("data", "output", "morb_csi.RDS"))

```

```{r param}

# Effect estimates reported per ring_div_fct doses in the ring
ring_div_fct <- 5000
...
morb_desc <- morb_rings_raw %>% 
  ...
  dplyr::select(grappe, arm, ring_range_text, ends_with("_doses")) %>%
  mutate(
    ring_range_text = factor(ring_range_text, levels = c("0 - 10 km", "10 - 20 km", "20 - 30 km")),
    total_doses = azithro_doses + placebo_doses,
...
ggsave(here("figs", "az-ring-model-results.png"),
