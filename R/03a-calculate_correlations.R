...
morb_tx_int %>%
  ...
  summarise(
    sd_tx_int = sd(tx_int, na.rm = TRUE),
    sd_avg_res = sd(avg_res, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(sd_tx_int == 0 | sd_avg_res == 0)
...
morb_ring %>%
  ...
  summarise(
    sd_n_doses = sd(n_doses, na.rm = TRUE),
    sd_avg_res = sd(avg_res, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(sd_n_doses == 0 | sd_avg_res == 0) %>%
  group_by(ab_class) %>%
  summarise(n = n())
